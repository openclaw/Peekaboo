...
extension RemoteCaptureGateOwnershipTests {
    ...
    func `Bridge without registered ScreenCaptureKit capability preserves automatic capture`() async throws {
        ...
        _ = try await client.desktopObservation(DesktopObservationRequest(
            target: .screen(index: 0),
...
extension RemoteCaptureGateOwnershipTests {
    ...
    func `Bridge owner runs concurrent background auto screen observations on modern capture`() async throws {
        ...
        async let first = firstClient.desktopObservation(request)
        async let second = secondClient.desktopObservation(request)
...
private final class BridgeAutoScreenObservationService: DesktopObservationServiceProtocol {
    ...
    init(failSlowAutomaticCapture: Bool = true) {
