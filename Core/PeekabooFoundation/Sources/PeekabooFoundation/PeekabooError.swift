...
extension PeekabooError {
    /// Whether this failure is raised during validation or lookup, before the automation stack can
    /// dispatch any desktop event (mouse, keyboard, AX action, window operation).
    ///
    /// Snapshot bookkeeping uses this to decide whether a failed command may have mutated the
    /// desktop: failures listed here must not invalidate cached UI snapshots, while everything else
    /// (timeouts, partial failures, unknown errors) conservatively counts as a possible mutation.
