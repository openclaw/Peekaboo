...
struct BackgroundTextInputDeliveryTests {
    ...
    private final class Fixture {
        let route: BackgroundTextInputRoute
        ...
        init(route: BackgroundTextInputRoute, strategy: UIInputStrategy) {
            self.route = route
        ...
        func run(_ payload: Payload) async throws -> TypeService.TypeActionExecutionSummary {
            let driver = TargetedTypeInputDriver(
                insertText: { _, _ in try self.tryAXWrite() },
                performTextKey: { _, _ in try self.tryAXWrite() ? .accessibilityValue : .unsupported },
                replaceText: { _, _ in try self.tryAXWrite() },
