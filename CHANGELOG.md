...
- Bind `action` and `set-value` snapshots, resolved AX elements, outcomes, and signed Bridge 1.37 results to one process generation; reject PID reuse, foreign elements, receiptless providers, old hosts, and targetless success before dispatch or retry.
