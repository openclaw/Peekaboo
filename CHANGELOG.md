...
- Scope process-local MCP and Agent browser refs to one caller, provider child epoch, page, snapshot, and document generation; atomically resolve, freshness-check, dispatch, and project them under one FIFO, reject raw-schema escapes and same-target cross-session ownership, and let independent targets progress concurrently while Bridge-backed opaque refs fail closed pending authenticated wire support.
