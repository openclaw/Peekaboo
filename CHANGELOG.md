...
- The typing caption shows the typed text verbatim (that is its job); typing into secure text fields is masked as bullets before the event is persisted or displayed (best-effort focus sampling before and after typing, scoped to the delivery process for background typing), and `PEEKABOO_VISUALIZER_MASK_TYPED_TEXT=true` masks everything for privacy-sensitive setups.
...
- Element highlights from senders running older Peekaboo binaries are now flipped by the receiver (payloads carry a coordinate-space marker), so stale CLIs and MCP servers stop drawing vertically mirrored rectangles; detection sheets also drop window-sized container rects, cap at 120 highlights per screen, and render in one overlay window per screen.
