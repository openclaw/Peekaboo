...
### Fixed
- Keep `capture action` sampling active across pre-roll, child execution, and post-roll, release only generation-attributed children after terminal-event admission, keep concurrent leader/descendant waits off the cooperative executor, terminate surviving process-group descendants before validation, reject replaced artifacts, compose focus and child receipts without inventing partial effects, and require Apple-anchored source-stamped host provenance.
- Warm ScreenCaptureKit ownership validation off the main actor before Bridge socket/capability publication so cold process-signature scans cannot consume concurrent capture request deadlines.
- Route background Bridge full-screen automatic capture directly through host-owned ScreenCaptureKit so concurrent observations do not exhaust the request deadline behind serial classic-helper timeouts.
...
- Scope process-local MCP and Agent browser refs to one caller, provider child epoch, page, snapshot, and document generation; reserve exact targets before permission-bearing setup, project provider-owned identifiers/private paths across every result surface without rewriting domain data, and let independent background session lanes overlap under durable cross-process invalidation while foreground setup, shared debt, same-target access, and Bridge-backed opaque refs remain fail closed.
