...
- Validate every registered command signature and default-subcommand target before routing, reject duplicate command, subcommand, option, and flag definitions deterministically instead of trapping or silently selecting one, and keep daemon `--bridge-socket` on the canonical runtime option.
...
- Preserve MCP tool failure status, content, structured values, and metadata through Agent generation instead of reshaping failures as successful results.
