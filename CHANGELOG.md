...
- Bind dialog input to an exact PID/generation/window/sheet/field through capability-gated Bridge protocol 1.27, add selector-preserving forced Escape dismissal in protocol 1.28, revalidate focus immediately before every global keyboard event without claiming causality, and keep forced dismissal retry-unsafe even if another controller closes the dialog.
