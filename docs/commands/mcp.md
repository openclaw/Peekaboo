...
- For a missing `query`, `click` uses `wait_for` (default 5000 ms, maximum 60000 ms) to inspect fresh Accessibility state in the snapshot's exact window. Each observation carries the original PID, process generation, window ID, and bounds; a changed receipt refuses the action. Polling captures no screenshots and does not focus the window. The click uses the element ID from the matching observation. `wait_for: 0` checks only the supplied snapshot; a missing snapshot-local `on` ID is always refused immediately and is never remapped.
