...
- `is_on_screen` reports WindowServer ordering, not foreground status, unobscured pixels, or successful page rendering. An omitted native visibility flag means false, including for hidden windows with ordinary display bounds. Such windows remain in the inventory and addressable by exact ID; observation eligibility and inventory completeness are separate from visibility.
