...
- Destructive state and geometry actions resolve broad selectors once, pin the selected window's CGWindowID plus owner PID/process-start identity, and revalidate that receipt after Bridge queue admission and around native dispatch/readback. A disappeared or recycled PID/window fails closed instead of retargeting a sibling or newer process.
