...
- `maximize` is idempotent. AppKit's zoom button is a toggle (pressing it on an already-maximized window restores the previous size), so `maximize` detects when the toggle un-maximized an already-maximized window and re-asserts the maximized state. Calling `maximize` on an already-maximized window leaves it maximized and prints `is already maximized`. The one edge case: an app whose zoom target is smaller than the whole screen (rare) is not recognized as "maximized", so a second `maximize` there still toggles.
