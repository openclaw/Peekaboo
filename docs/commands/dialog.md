...
- Targeted structural discovery reads Accessibility nodes on a bounded background worker lane, with one three-second hierarchy deadline, at most 512 nodes and depth 32, and cancellation checks. Stalled reads cannot hold the main actor or publish late candidates; unreadable or truncated classification refuses before any mutation. An abandoned native read retains its worker slot until it actually returns, so another discovery for that process generation can refuse meanwhile. These bounds cover structural discovery, not subsequent metadata extraction or action verification.
