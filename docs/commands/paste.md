...
- When the automation service reports a canonical no-dispatch refusal with no prior focus effect or attempted clipboard write, paste preserves implicit observations, including for background text. Partial clipboard writes and unclassified input failures still invalidate conservatively, and another operation's pending mutation barrier is never canceled by this refusal.
