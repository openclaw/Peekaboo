...
- Background scrolling first uses an owned axis-matching `AXScrollBar` with a writable finite numeric value and valid range. Each requested unit uses the bar's positive finite `AXValueIncrement`, or one tenth of its range when none is usable, clamped to the range; the combined value update counts as one native mutation. This is the same scrollbar amount calculation as before, now preferred over page actions that some native scroll areas advertise but cannot perform. A nested scroll area's bar is never borrowed. If no numeric route is available, directional page actions and then scrollbar increment/decrement actions remain available.
