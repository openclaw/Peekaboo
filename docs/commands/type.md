...
- The legacy SDK `type(text:target:clearExisting:...)` also defaults to `actionFirst`. A named-element replacement may use one AX edit without a focus click or keystroke cadence. Its nil-target form and the CLI's foreground action-array form retain keyboard delivery; select an explicit synthetic strategy when keyboard semantics are required.
