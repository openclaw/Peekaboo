...
| `--input-strategy <strategy>` | Override typing delivery: `actionFirst` (default), `actionOnly`, `synthFirst`, or `synthOnly`. Per-call overrides execute locally. |
...
- The legacy SDK `type(text:target:clearExisting:typingDelay:snapshotId:)` also defaults to `actionFirst`, while preserving named-target focus and requested per-character pacing. One AX replacement is eligible only with `clearExisting: true`, zero `typingDelay`, and a fresh check proving the named target is the current keyboard receiver; cached focus or a frontmost app/window alone is insufficient. A requested positive delay or a successfully read focus mismatch lets `actionFirst` use the existing synthetic focus/clear/type path; `actionOnly` refuses before dispatch. An unreadable or uncertain focus check stops before input under either action strategy, never falls back, and is not treated as an unsupported action. Explicit synthetic strategies, the SDK's nil-target keyboard fallback, and the CLI's foreground action-array delivery are unchanged.
...
## Troubleshooting
- Verify Screen Recording + Accessibility permissions (`peekaboo permissions status`). Background keyboard-event delivery also requires Event Synthesizing access for the sending process; native Accessibility edits do not. Request it when needed with `peekaboo permissions request event-synthesizing`.
