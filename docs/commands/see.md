...
| `--no-elements` | Skip element detection for the cheapest pixel path. An exact `--window-id` capture still returns `snapshot_id` with a coordinate-only receipt for background clicks; screen, area, frontmost, and multi captures do not. |
...
- `ACCESSIBILITY_INCOMPLETE` means the exact target exists but its AX-only read was explicitly incomplete and returned no usable elements. Retry the same exact observation once for fresh evidence; if it persists, use screenshot/OCR. This code never means the element is absent and never substitutes for `TIMEOUT`.
