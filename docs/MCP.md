...
Process-targeted MCP `type`, `press`, `paste`, and element `click` calls retain one application process-generation receipt instead of relying on a reusable numeric PID. Type/paste validate before each emitted unit, press retains the same receipt across the full sequence, and clicks validate around dispatch. MCP and Agent runtime selection require Bridge protocol 1.22 for these process-only routes; older hosts are rejected before input rather than being allowed to ignore the receipt.
