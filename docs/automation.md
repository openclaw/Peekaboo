...
When a field has no stable Accessibility element, `type --at x,y --snapshot <id>` can focus one pixel and type under a single exact-window background lane. It never activates the app or splits the click and keyboard units into independently retryable actions. Foreground modifier-click remains explicit: `click --on <id> --snapshot <id> --foreground --modifiers cmd,shift` restores only cursor/focus state that still matches Peekaboo's own last write, so concurrent user input wins.
