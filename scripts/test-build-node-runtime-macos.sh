...
printf '#include <stdio.h>\nint main(void){puts("v24.21.0");return 0;}\n' > "$TEST_DIR/node.c"
printf 'fixture license\n' > "$TEST_DIR/LICENSE"

for pair in arm64:arm64 x64:x86_64; do
  label="${pair%%:*}"
  arch="${pair#*:}"
  root="$TEST_DIR/node-v24.21.0-darwin-$label"
  mkdir -p "$root/bin"
  /usr/bin/clang -arch "$arch" "$TEST_DIR/node.c" -o "$root/bin/node"
  cp "$TEST_DIR/LICENSE" "$root/LICENSE"
  /usr/bin/tar -czf "$TEST_DIR/$label.tar.gz" -C "$TEST_DIR" "node-v24.21.0-darwin-$label"
done

sha() { /usr/bin/shasum -a 256 "$1" | /usr/bin/awk '{print $1}'; }
output="$TEST_DIR/PeekabooQualificationNode.app"
/usr/bin/lipo -create \
  "$TEST_DIR/node-v24.21.0-darwin-arm64/bin/node" "$TEST_DIR/node-v24.21.0-darwin-x64/bin/node" \
...
NODE_RUNTIME_ARM64_BINARY_SHA="$(sha "$TEST_DIR/node-v24.21.0-darwin-arm64/bin/node")" \
NODE_RUNTIME_X64_BINARY_SHA="$(sha "$TEST_DIR/node-v24.21.0-darwin-x64/bin/node")" \
...
jq -e '
  .runtime_version == "24.21.0" and .identifier == "boo.peekaboo.qualification-node" and
