...
/usr/libexec/PlistBuddy -c 'Set :CFBundleShortVersionString 24.21.0' "$node_app/Contents/Info.plist"
...
jq -n --arg binarySHA "$node_binary_sha" --argjson binarySize "$node_binary_size" \
 ...
 {version:1,runtime_version:"24.21.0",identifier:"boo.peekaboo.qualification-node",
...
/usr/bin/ditto -c -k --norsrc --keepParent "$node_app" "$candidate/PeekabooQualificationNode-24.21.0.app.zip"
...
playground_zip_sha="$(sha "$candidate/Playground-$VERSION.app.zip")"; node_zip_sha="$(sha "$candidate/PeekabooQualificationNode-24.21.0.app.zip")"
...
jq -n --arg version "$VERSION" --arg source "$SOURCE_COMMIT" --arg lockSHA "$lock_sha" \
 ...
 --arg nodeSHA "$node_zip_sha" --argjson nodeSize "$(size "$candidate/PeekabooQualificationNode-24.21.0.app.zip")" \
...
(cd "$candidate" && /usr/bin/shasum -a 256 peekaboo-macos-universal.tar.gz "Peekaboo-$VERSION.app.zip" \
 "Peekaboo-$VERSION.dmg" "Playground-$VERSION.app.zip" PeekabooQualificationNode-24.21.0.app.zip \
