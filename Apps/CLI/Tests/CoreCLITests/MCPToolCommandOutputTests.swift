...
struct MCPToolCommandOutputTests {
    ...
    func `Browser CLI envelope projects canonical failure and exact target metadata`() throws {
        ...
        #expect(envelope.target_identity?.processIdentifier == 4242)
        #expect(envelope.target_identity?.processStartIdentityDecimal == "9007199254740993")
        #expect(envelope.target_identity?.windowID == 73)
