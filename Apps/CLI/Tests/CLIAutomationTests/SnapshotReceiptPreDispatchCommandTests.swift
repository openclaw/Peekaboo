...
struct SnapshotReceiptPreDispatchCommandTests {
    @Test
    func `process scoped element actions refuse missing generation with reusable snapshot semantics`() async throws {
        let snapshots = StubSnapshotManager()
        let snapshotID = try await Self.storeProcessWithoutGenerationElementSnapshot(in: snapshots)
        ...
        for arguments in [
            ...
            #expect((error["message"] as? String)?.contains("process-generation") == true)
    ...
    private static func storeProcessElementSnapshot(
        ...
        try await snapshots.storeDetectionResult(
            ...
            result: AutomationTestFixtures.detectionResult(
                snapshotID: snapshotID,
                screenshotPath: "/tmp/process-without-generation.png",
                elements: DetectedElements(
                    buttons: [button],
                    textFields: [textField]
                ),
