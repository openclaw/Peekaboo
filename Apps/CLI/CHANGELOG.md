...
- Make Agent sessions immutable background-only by default, centrally refusing foreground/global input, activation, shared-pointer, persistent clipboard mutation, Dock/Space, browser setup/fronting, and Shell bypasses before dispatch; `--allow-foreground` sets a new session's immutable maximum but must be explicitly repeated on resume and never grants shell authority, while session output exposes full copyable IDs, tasks, lifecycle meaning, and stored policy.
