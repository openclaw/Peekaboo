...
- Scope process-local persistent MCP and Agent browser refs to one caller, provider child epoch, page, snapshot, and document generation; reserve targets before provider setup, sanitize provider-owned identifiers/private paths on every result surface without rewriting domain data, and let different background session lanes overlap under durable cross-process invalidation while foreground setup, shared debt, same-target access, and Bridge-backed opaque refs remain fail closed.
