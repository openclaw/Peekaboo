...
- Scope process-local persistent MCP and Agent browser refs to one caller, provider child epoch, page, snapshot, and document generation; atomically reject stale or raw-schema-escaped refs before mutation while Bridge-backed opaque refs fail closed pending authenticated wire support.
